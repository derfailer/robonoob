...
public class SoccerThinking1 {
   ...
   */
  public SoccerThinking1(PerceptorInput percIn, LocalFieldView localView,
...
//          log.log("3. the ball is not in front of the robot. ") ;
        ...
        else if ((serverTime - oppGoalRPost.getTimeStamp() < lookTime)
                && (serverTime - oppGoalLPost.getTimeStamp() < lookTime)) {
          log.log("5. the robot has the actual goal coordinates");

          // if the ball does not lie between the robot and the goal
          if ((oppGoalRPost.getCoords().getAlpha() <= ballCoords.getAlpha())
                  || (oppGoalLPost.getCoords().getAlpha() >= ballCoords.getAlpha())) {
